...
// For now, use mock data when API_BASE_URL is not defined
export async function fetchApi<T>(path: string, options?: RequestInit): Promise<T> {
  if (!API_BASE_URL) {
    // In development, if API_BASE_URL is not set, we'll use Next.js API routes
    // which should handle the mock data
